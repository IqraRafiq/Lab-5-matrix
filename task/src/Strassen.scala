...
object Strassen {
	def main(args: Array[String]): Unit = {
		print("Enter the power factor (2^n | n is power factor): ")
		val orderPower: Int = readInt()
		val matrixOrder: Int = Math.pow(2, orderPower).intValue

		val matrixA = Array.fill(matrixOrder, matrixOrder)(scala.util.Random.nextInt(50))
		val matrixB = Array.fill(matrixOrder, matrixOrder)(scala.util.Random.nextInt(50))
		...
		printArray(sMultiply(matrixA, matrixB))
	}
	
	def printArray[Int](givenArray: Array[Array[Int]]) = {
		for (outer <- 0 until givenArray.length){
			for (inner <- 0 until givenArray.length){
				print(givenArray(outer)(inner) + "\t")
			}
			print("\n")
		}
	}
